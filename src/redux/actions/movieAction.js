import api from "../api"
    ...
    //             const detailApi=api.get(`https://api.themoviedb.org/3/movie/${id}?api_key=${API_KEY}&language=en-US`)
    //             let [MovieDetails] = await Promise.all([detailApi])
...
function getMovies(){
    ...
    return async(dispatch)=>{
        try{
            ...
            // (장점)axios를 쓰면 항상 data라는 필드에 받은 응답을 알아서 넣어준다. 
